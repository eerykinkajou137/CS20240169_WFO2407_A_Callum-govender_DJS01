...
function validateParameters(velocity, acceleration, time, fuel, fbr) {
  ...
  if (velocity < 0 || acceleration < 0 || time < 0 || fuel < 0 || fbr < 0) { // Check for non-negative and numeric values
    throw new Error("All parameters should not be negative values.");
  }

  // Check if all parameters are of type 'number'
  if (
    typeof velocity !== 'number' || typeof acceleration !== 'number' || typeof time !== 'number' || typeof fuel !== 'number' || typeof fbr !== 'number' ) {
